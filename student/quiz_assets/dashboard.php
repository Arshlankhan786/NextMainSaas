...
foreach ($ranking as $student_id => $data) {
    if (count($top_students) >= 10) break;
...
$monthly_attendance = (int)safeVal($conn,
    ...
    'cnt', 0);
$total_working_days = (int)safeVal($conn,
...
try {
    ...
    if ($tp_res) {
        while ($tp = $tp_res->fetch_assoc()) {
            // Date-based current day number
            $tp['current_day'] = max(1, (int)((strtotime($today) - strtotime($tp['start_date'])) / 86400) + 1);
            // Check today log
            $tl = $conn->query("SELECT id FROM project_daily_logs WHERE project_id={$tp['id']} AND log_date='$today'");
            $tp['has_today'] = ($tl && $tl->num_rows > 0);
            if (!$tp['has_today']) $task_overdue = true;
            // Last 3 logs
            $l3 = $conn->query("SELECT * FROM project_daily_logs WHERE project_id={$tp['id']} ORDER BY log_date DESC LIMIT 3");
            $tp['last3'] = [];
            if ($l3) while ($lr = $l3->fetch_assoc()) $tp['last3'][] = $lr;
            $task_projects[] = $tp;
        }
    }
...
if ($start_date && $duration_months > 0) {
    try {
        $end_date = (new DateTime($start_date))
            ...
            ->format('Y-m-d');
    } catch (Exception $e) { $end_date = null; }
}
...
if ($start_date && $end_date) {
    ...
    } catch (Exception $e) { $duration_months = ''; }
}

$current_month_check = date('Y-m');

$paid_this_month = (int)safeVal($conn,
    "SELECT COUNT(*) AS cnt
     FROM payments
     WHERE student_id = $sid
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar-item.active { background: rgba(124,58,237,0.25); color: #fff; }
        ...
        .card-glass:hover { border-color: var(--border2); }
        ...
        .greeting-text { font-family: 'Poppins', sans-serif; }
        .greeting-text h1 {
            font-size: 20px;
            font-weight: 700;
            line-height: 1.2;
        ...
        .greeting-text p { color: var(--text-muted); font-size: 13px; margin-top: 2px; }
        ...
        .btn-check-in:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 22px rgba(16,185,129,0.4); }
        .btn-check-out {
            background: linear-gradient(135deg, #dc2626, #ef4444);
            color: #fff;
            box-shadow: 0 4px 16px var(--red-glow);
        }
        .btn-check-out:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 22px rgba(239,68,68,0.4); }
        ...
        #att-toast.toast-success { border-color: rgba(16,185,129,0.55); }
        #att-toast.toast-error   { border-color: rgba(239,68,68,0.55); }
        ...
        .notif-btn:hover { color: var(--purple-light); border-color: var(--purple); }
        .notif-dot {
        ...
        .rank-label { font-size: 10px; color: var(--text-muted); font-weight: 500; letter-spacing: 0.8px; text-transform: uppercase; }
        .rank-pts { font-family: 'Poppins', sans-serif; font-size: 18px; font-weight: 700; color: var(--purple-light); }
        .rank-pts-label { font-size: 10px; color: var(--text-muted); text-align: right; letter-spacing: 0.6px; text-transform: uppercase; }

        /* ─── INFO BOXES ─── */
        .info-boxes { display: flex; flex-direction: column; gap: 7px; }
        ...
        .info-box-label { color: var(--text-muted); font-weight: 500; display: flex; align-items: center; gap: 6px; }
        .info-box-label i { font-size: 11px; width: 14px; }
        .info-box-val { font-weight: 700; font-size: 12.5px; color: var(--text); }
        .info-box-val.green { color: var(--green); }
        .info-box-val.red { color: #f87171; }
        .info-box-val.yellow { color: var(--yellow); }
        ...
        .stat-mini.s-purple { background: rgba(124,58,237,0.1); border: 1px solid rgba(124,58,237,0.2); }
        .stat-mini.s-purple::before { background: var(--purple); }
        .stat-mini.s-green  { background: rgba(16,185,129,0.08); border: 1px solid rgba(16,185,129,0.18); }
        .stat-mini.s-cyan   { background: rgba(6,182,212,0.08);  border: 1px solid rgba(6,182,212,0.18); }
        .stat-mini.s-yellow { background: rgba(245,158,11,0.08); border: 1px solid rgba(245,158,11,0.18); }
        ...
        .s-purple .stat-mini-icon { background: rgba(124,58,237,0.2); color: var(--purple-light); }
        .s-green  .stat-mini-icon { background: rgba(16,185,129,0.15); color: var(--green); }
        .s-cyan   .stat-mini-icon { background: rgba(6,182,212,0.15);  color: var(--cyan); }
        .s-yellow .stat-mini-icon { background: rgba(245,158,11,0.15); color: var(--yellow); }
        ...
        .s-purple .stat-mini-val { color: var(--purple-light); }
        .s-green  .stat-mini-val { color: var(--green); }
        .s-cyan   .stat-mini-val { color: var(--cyan); }
        .s-yellow .stat-mini-val { color: var(--yellow); }

        .stat-mini-label { font-size: 11px; color: var(--text-muted); font-weight: 500; }
        .stat-mini-sub { font-size: 10px; color: var(--text-muted); opacity: 0.7; }
        ...
        .att-big-pct span { font-size: 22px; letter-spacing: 0; }
        .att-meta { font-size: 12px; color: var(--text-muted); }
        .att-meta strong { color: var(--text); font-size: 14px; }
        .att-bar-wrap { flex: 1; margin: 0 20px; }
        .att-bar-track {
        ...
        .circle-card.c-purple { background: rgba(124,58,237,0.08); border: 1px solid rgba(124,58,237,0.2); }
        .circle-card.c-blue   { background: rgba(6,182,212,0.08);  border: 1px solid rgba(6,182,212,0.18); }
        ...
        .circle-track { stroke: rgba(255,255,255,0.07); }
        .circle-fill {
            stroke-dashoffset: 226;
            stroke-linecap: round;
            transition: stroke-dashoffset 1.4s cubic-bezier(0.34,1.56,0.64,1);
        }
        .c-purple .circle-fill { stroke: url(#gradPurple); }
        .c-blue   .circle-fill { stroke: url(#gradCyan); }
        ...
        .c-purple .circle-pct { color: var(--purple-light); }
        .c-blue   .circle-pct { color: var(--cyan); }
        .circle-pct-sym { font-size: 10px; font-weight: 500; }
        ...
        .circle-info p { font-size: 11.5px; color: var(--text-muted); line-height: 1.5; }
        ...
        .emi-info p { font-size: 11.5px; color: var(--text-muted); margin-top: 3px; }
        .emi-info strong { color: var(--text); }
        .emi-badge {
            margin-left: auto;
            background: rgba(239,68,68,0.15);
        ...
        .lb-item.is-me .lb-avatar { background: linear-gradient(135deg, #7c3aed, #06b6d4); }
        .lb-avatar img {
        ...
        .lb-item.is-me .lb-pts { color: var(--cyan); }
...
<nav class="sidebar">
    ...
    <a href="my_tests.php" class="sidebar-item <?php echo in_array($current_page, ['my_tests','take_quiz','quiz_result'])?'active':''; ?>">
        <i class="fas fa-file-alt"></i>
        <span class="sidebar-tooltip">My Tests</span>
    </a>
    <a href="typing_competition.php" class="sidebar-item <?php echo $current_page==='typing_competition'?'active':''; ?>">
...
<div class="app-shell">
    ...
    <div class="top-bar fade-up">
        <div class="greeting-text">
            <h1>Good <?php echo (date('H')<12)?'Morning':((date('H')<18)?'Afternoon':'Evening'); ?>, <?php echo htmlspecialchars(explode(' ', $student_data['full_name'])[0]); ?> 👋</h1>
            <p><?php echo date('l, d F Y'); ?> &nbsp;·&nbsp; <?php echo htmlspecialchars($student_data['student_code'] ?? ''); ?></p>
        </div>
        <div class="top-actions">
            ...
            <div class="att-btn-group">
                <button id="btnCheckIn"
                        class="btn-check-in"
                        onclick="doCheckIn()"
                        <?php echo $already_checked_in ? 'disabled' : ''; ?>>
            ...
            <div class="att-btn-group">
                <button id="btnCheckOut"
                        class="btn-check-out"
                        onclick="doCheckOut()"
                        <?php echo ($already_checked_out || !$already_checked_in) ? 'disabled' : ''; ?>>
                    <i class="fas fa-sign-out-alt"></i>
    ...
    <div class="main-grid">
        ...
        <div class="card-glass profile-card fade-up delay-1">
            ...
            <div class="avatar-wrap">
                ...
                <div>
                    ...
                    <div class="student-course"><?php echo htmlspecialchars($course_data['name'] ?? 'No Course Assigned'); ?></div>
                </div>
            </div>
            ...
            <div class="rank-badge">
                <div>
                    ...
                    <div class="rank-num">#<?php echo $my_rank > 0 ? str_pad($my_rank,2,'0',STR_PAD_LEFT) : '--'; ?></div>
                </div>
            ...
            <div class="info-boxes">
                ...
                <div class="info-box">
                    ...
                    <span class="info-box-val <?php echo $outstanding>0?'red':'green'; ?>">₹<?php echo number_format($outstanding,0); ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="center-col">
            ...
            <div class="attendance-hero fade-up delay-2">
                <div>
                    <div style="font-size:11px;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px;font-weight:600;margin-bottom:4px;">Attendance Rate</div>
                    <div class="att-big-pct"><?php echo $attendance_percent; ?><span>%</span></div>
                </div>
                <div class="att-bar-wrap">
                    <div style="font-size:12px;color:var(--text-muted);"><?php echo $monthly_attendance; ?> / <?php echo $total_working_days; ?> days &nbsp;·&nbsp; <?php echo date('F Y'); ?></div>
                ...
                <div style="text-align:right;">
                    ...
                    <div style="font-size:13px;font-weight:700;color:<?php echo $attendance_percent>=75?'var(--green)':'var(--red)'; ?>;margin-top:2px;">
            ...
            <div class="circles-row fade-up delay-3">
                ...
                <div class="circle-card c-purple card-glass">
                    ...
                    <div class="circle-wrap">
                        <svg class="circle-svg" width="72" height="72" viewBox="0 0 72 72">
                            ...
                            <circle class="circle-fill" id="courseCircle" cx="36" cy="36" r="30" fill="none" stroke-width="6" stroke-dasharray="188.5" stroke-dashoffset="188.5"/>
                        </svg>
                ...
                <div class="circle-card c-blue card-glass">
                    <div class="circle-wrap">
                        <svg class="circle-svg" width="72" height="72" viewBox="0 0 72 72">
                            <circle class="circle-track" cx="36" cy="36" r="30" fill="none" stroke-width="6"/>
                            <circle class="circle-fill" id="feesCircle" cx="36" cy="36" r="30" fill="none" stroke-width="6" stroke-dasharray="188.5" stroke-dashoffset="188.5"/>
                        </svg>
                        <div class="circle-label">
                    ...
                    <div class="circle-info">
                        <h4>Fees Paid</h4>
                        <p>₹<?php echo number_format($paid_amount,0); ?> of<br>₹<?php echo number_format($total_fees,0); ?> cleared</p>
                    </div>
                </div>
            </div>
                ...
                <div class="emi-info">
                    ...
                    <p>
                        ...
                        <?php if($next_due_date): ?>
                         &nbsp;·&nbsp; Next due: <strong><?php echo date('d M Y', strtotime($next_due_date)); ?></strong>
                        <?php endif; ?>
                        <?php else: ?>
            ...
            ">
                <!-- Header -->
                <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid rgba(99,102,241,.15);">
                    <div style="font-size:13px;font-weight:800;color:#e0e7ff;display:flex;align-items:center;gap:8px;">
                        <i class="fas fa-clipboard-list" style="color:#a78bfa;"></i> Live Project Tracker
                    </div>
                    <a href="task_manager.php" style="font-size:11px;font-weight:700;color:#818cf8;text-decoration:none;">
                ...
                <!-- Project Row -->
                <div style="padding:12px 16px;border-bottom:1px solid rgba(99,102,241,.1);<?= (!$tp['has_today']) ? 'background:rgba(239,68,68,.04);' : '' ?>">
                    <!-- Project Info Row -->
                    <div style="display:grid;grid-template-columns:1fr auto auto auto auto;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:10px;">
                        ...
                        <div>
                            <div style="font-size:13px;font-weight:800;color:#e0e7ff;">
                                ...
                                <?php if (!$tp['has_today']): ?>
                                <span style="font-size:9px;font-weight:700;background:rgba(239,68,68,.15);color:#f87171;border:1px solid rgba(239,68,68,.3);padding:2px 7px;border-radius:99px;margin-left:6px;">
                        ...
                        <div style="text-align:center;">
                            <div style="font-size:9px;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.5px;">Start Date</div>
                            <div style="font-size:12px;font-weight:700;color:#c7d2fe;"><?= date('d-m-Y', strtotime($tp['start_date'])) ?></div>
                        </div>
                        <!-- Current Day -->
                        <div style="text-align:center;">
                            <div style="font-size:9px;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.5px;">Day</div>
                            <div style="font-size:18px;font-weight:900;color:#a78bfa;line-height:1;"><?= $tp['current_day'] ?></div>
                        </div>
                        <!-- Update Today Work -->
                        <div>
                            <button onclick="openUpdateModal(<?= $tp['id'] ?>, '<?= htmlspecialchars($tp['project_name'], ENT_QUOTES) ?>')"
                    ...
                    <div style="background:rgba(0,0,0,.15);border-radius:10px;padding:10px 12px;">
                        <div style="font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:.7px;color:rgba(255,255,255,.4);margin-bottom:8px;">
                            <i class="fas fa-history" style="margin-right:4px;color:#818cf8;"></i>Last 3 Activities
                        </div>
                        <?php if (empty($tp['last3'])): ?>
                        <div style="font-size:11.5px;color:rgba(255,255,255,.3);text-align:center;padding:6px 0;">No project updates yet.</div>
                        <?php else: ?>
                        <?php foreach ($tp['last3'] as $log): ?>
                        <div style="display:grid;grid-template-columns:85px 1fr 55px;gap:8px;padding:5px 0;border-bottom:1px solid rgba(255,255,255,.04);font-size:11.5px;align-items:start;">
                            <span style="color:#60a5fa;font-weight:700;white-space:nowrap;"><?= date('d-m-Y', strtotime($log['log_date'])) ?></span>
                            <span style="color:#d1d5db;line-height:1.4;word-break:break-word;"><?= htmlspecialchars(mb_strimwidth($log['work_description'], 0, 85, '…')) ?></span>
                            <span style="text-align:right;">
                                <span style="font-size:10px;font-weight:800;color:#a78bfa;background:rgba(99,102,241,.15);padding:2px 7px;border-radius:99px;">Day <?= $log['day_number'] ?></span>
                            </span>
                        </div>
                        <?php endforeach; ?>
                        <?php endif; ?>
                        <!-- Show Full Update button -->
                        <button onclick="openDashTimeline(<?= $tp['id'] ?>, '<?= htmlspecialchars($tp['project_name'], ENT_QUOTES) ?>')"
                ...
                <div style="text-align:center;padding:24px;color:rgba(255,255,255,.3);font-size:12px;">
                    <i class="fas fa-clipboard-list" style="display:block;font-size:28px;margin-bottom:8px;opacity:.4;"></i>
...
<aside class="right-panel">
    ...
    <div class="lb-list">
        ...
        <div style="text-align:center;color:var(--text-muted);font-size:12px;padding:20px 0;">No ranking data yet</div>
        <?php endif; ?>
    </div>
...
<div id="dashUpdateModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.75);backdrop-filter:blur(4px);align-items:center;justify-content:center;">
    <div style="background:#1e1533;border:1px solid rgba(99,102,241,.25);border-radius:16px;width:90%;max-width:480px;
        ...
        <style>@keyframes dashModalIn{from{transform:translateY(30px);opacity:0}to{transform:translateY(0);opacity:1}}</style>
        <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid rgba(99,102,241,.15);">
            ...
            <button onclick="closeDashUpdateModal()" style="background:none;border:none;color:#94a3b8;font-size:18px;cursor:pointer;line-height:1;padding:2px 6px;border-radius:6px;" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='#94a3b8'">×</button>
        </div>
        <div style="padding:16px 18px;">
            <input type="hidden" id="dashUpdateProjId">
            <p id="dashUpdateInfo" style="font-size:11px;color:#94a3b8;margin-bottom:10px;"></p>
            <label style="font-size:11.5px;font-weight:700;color:#d1d5db;margin-bottom:6px;display:block;">What did you work on today? *</label>
            <textarea id="dashUpdateWork" rows="5" placeholder="Describe your progress in detail..."
                ...
                onfocus="this.style.borderColor='#818cf8'" onblur="this.style.borderColor='rgba(99,102,241,.2)'"></textarea>
        </div>
        <div style="padding:10px 18px;border-top:1px solid rgba(99,102,241,.12);display:flex;justify-content:flex-end;gap:8px;">
            <button onclick="closeDashUpdateModal()" style="background:rgba(255,255,255,.06);color:#94a3b8;border:1px solid rgba(255,255,255,.1);padding:8px 16px;border-radius:9px;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;">Cancel</button>
            <button id="dashUpdateSubmitBtn" onclick="submitDashUpdate()" style="background:linear-gradient(135deg,#7c3aed,#6366f1);color:#fff;border:none;padding:8px 18px;border-radius:9px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;gap:6px;">
...
<!-- ══ FULL TIMELINE MODAL ══ -->
<div id="dashTimelineModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.75);backdrop-filter:blur(4px);align-items:center;justify-content:center;">
    <div style="background:#1e1533;border:1px solid rgba(99,102,241,.25);border-radius:16px;width:90%;max-width:620px;max-height:88vh;display:flex;flex-direction:column;
        box-shadow:0 20px 60px rgba(0,0,0,.6);animation:dashModalIn .22s ease;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid rgba(99,102,241,.15);flex-shrink:0;">
            <div style="font-size:13px;font-weight:800;color:#fff;">
                <i class="fas fa-list-ul" style="color:#a78bfa;margin-right:7px;"></i>
                <span id="dashTimelineTitle">Project Timeline</span>
            </div>
            <button onclick="closeDashTimeline()" style="background:none;border:none;color:#94a3b8;font-size:18px;cursor:pointer;line-height:1;padding:2px 6px;border-radius:6px;" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='#94a3b8'">×</button>
        </div>
        <div id="dashTimelineContent" style="padding:16px 18px;overflow-y:auto;flex:1;"></div>
        <div style="padding:10px 18px;border-top:1px solid rgba(99,102,241,.12);display:flex;justify-content:flex-end;flex-shrink:0;">
            <button onclick="closeDashTimeline()" style="background:rgba(255,255,255,.06);color:#94a3b8;border:1px solid rgba(255,255,255,.1);padding:7px 16px;border-radius:9px;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;">Close</button>
        </div>
    </div>
</div>
